...
### 10а. Бот падает с `password authentication failed for user "aemr"`

Эта ошибка означает, что пароль в `.env` не совпадает с паролем, который **сейчас** хранится в базе. Контейнер `db` инициализирует пользователя `aemr` из переменной `POSTGRES_PASSWORD` **только при первом запуске**, когда volume `db_data` ещё пустой. На последующих запусках Postgres **игнорирует** `POSTGRES_PASSWORD`. Если поменять значение в `.env` после первого запуска — бот будет ломиться с новым паролем, а БД хранит старый.

**Вариант А — снести данные и инициализировать заново.** Подходит на тестовом стенде. **Все обращения, операторы и настройки в БД пропадут.**

```bash
cd ~/aemr-bot/infra
docker compose down -v   # ключ -v снесёт named volumes, включая db_data
docker compose up -d --build
...
```bash
# 1. Подставить в .env старый пароль (тот, с которым volume был инициализирован).
...
# Ожидание: миграции прошли, бот стартовал, ошибок аутентификации нет.
```
