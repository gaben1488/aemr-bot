# Политика конфиденциальности при обработке персональных данных в информационной системе «Чат-бот MAX»
...
> **Подготовлено:** 2026-05-10, технические факты повторно сверены с кодом на 2026-07-09 (HEAD `main`; состав ПДн — `bot/aemr_bot/db/models.py` User/Appeal/Message, сроки хранения — `bot/aemr_bot/services/cron.py` retention-задачи, права субъекта — `bot/aemr_bot/handlers/{start.py,menu.py}`; новых отклонений не выявлено).
...
>
> Источник истины для повседневной выдачи жителю — `docs/Политика.md` → `docs/PRIVACY.pdf` (генерируется `scripts/generate_privacy_pdf.py`). Расширенная версия (настоящий документ) заменит её после утверждения распоряжением.
...
1.1.1. Чат-бот — дополнительный сервис приёма сообщений жителей для оперативного реагирования Администрации. Он не является каналом подачи официальных обращений граждан в порядке Федерального закона от 02.05.2006 № 59-ФЗ «О порядке рассмотрения обращений граждан Российской Федерации»; сроки и порядок, установленные Федеральным законом № 59-ФЗ, к сообщениям, поступившим через Чат-бот, не применяются (подробнее — пункт 4.3). Официальное обращение по Федеральному закону № 59-ФЗ можно подать письменно или через официальные каналы Администрации — электронную приёмную (`{electronic_reception_url}`) либо личный приём.
...
4.3. Основным правовым основанием обработки является пункт 2 части 1 статьи 6 Федерального закона № 152-ФЗ — обработка необходима для осуществления и выполнения возложенных на Администрацию законодательством Российской Федерации функций (функция работы с сообщениями из открытых источников и информирования населения возложена Федеральным законом от 09.02.2009 № 8-ФЗ и Уставом); согласие субъекта (пункты 4.1–4.2) является дополнительным основанием. Сообщение, поступившее через Чат-бот, является сообщением из открытого источника (Постановление Администрации Елизовского муниципального округа от 19.12.2025 № 2129) и не является обращением в значении статьи 4 Федерального закона № 59-ФЗ; положения Федерального закона № 59-ФЗ к такому сообщению не применяются (см. также пункт 1.1.1). Пункт 4 части 1 статьи 6 Федерального закона № 152-ФЗ (обработка при предоставлении государственных и муниципальных услуг по Федеральному закону № 210-ФЗ) в качестве основания не используется, поскольку Чат-бот является каналом обратной связи и информирования, а не предоставления муниципальной услуги.
...
7.5. **Журнал действий (`audit_log`)** — хранится 1 год (365 дней) с момента записи (параметр AUDIT_LOG_RETENTION_DAYS) как доказательная база для проверок Роскомнадзора и иных регулирующих органов; срок установлен в соответствии с принципом ограничения хранения персональных данных (часть 7 статьи 5 Федерального закона № 152-ФЗ — хранение не дольше, чем требуют цели обработки). Очистка выполняется автоматически ежедневно.
...
8.2. При обработке сообщения, требующего привлечения профильного подразделения Администрации или подведомственного муниципального учреждения, Оператор передаёт необходимые сведения в порядке, установленном разделом 8 Регламента (взаимодействие с профильными подразделениями при подготовке ответов), с соблюдением требований Федерального закона № 152-ФЗ. Передача персональных данных третьих лиц, не относящихся непосредственно к существу поставленного вопроса, не допускается.
...
11.3. Уведомления об инциденте готовятся ответственными должностными лицами Администрации; автоматическая фиксация такого инцидента в журнале Чат-бота не предусмотрена. Связанные записи журнала действий `audit_log` хранятся в соответствии с пунктом 7.5 (1 год, 365 дней).
...
- [ ] Правовая рамка по Регламенту v8: основание обработки ПДн — п.2 ч.1 ст.6 152-ФЗ (раздел 4.3); 59-ФЗ к сообщениям через Чат-бот не применяется (сообщение из открытого источника, Постановление № 2129); передача в профильные подразделения — по разделу 8 Регламента (раздел 8.2).
...
- [ ] Пункт 7.5: 1 год (365 дней) по `audit_log` — реализовано в `cron.py::_job_audit_log_retention`, параметр `AUDIT_LOG_RETENTION_DAYS` (`config.py`). Сверить актуальность значения на дату публикации.
