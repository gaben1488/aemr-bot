...
| 11 Бэкап и фоновые задачи (Таблица 3) | 3 + 17 строк | 17 | 0 | 0 |
...
### Таблица 3 — Фоновые задачи (16 обязательных + 1 опциональная)
...
| 6 | Напоминания об открытых обращениях | Пн-пт 09:00-11:59 и 13:00-17:59 :10 | `open-reminder-workhours` cron `day_of_week=mon-fri, hour="9-11,13-17", minute=10` |
| 7 | Напоминания о просроченных | Пн-пт 09:00-11:59 и 13:00-17:59 :40 | `overdue-reminder-workhours` cron `day_of_week=mon-fri, hour="9-11,13-17", minute=40` |
| 8 | Ежемесячный отчёт | 1-го 09:00 | `monthly-stats` cron `day=1, hour=9` |
| 9 | Сигнал жизни (базовый) | Каждый час 24/7 :05 | `pulse-hourly` cron `minute=5` |
| 10 | Сигнал жизни (рабочее время) | Пн-пт 09-17 :35 | `pulse-workhours-extra` cron `day_of_week=mon-fri, hour=9-17, minute=35` |
| 11 | Сигнал старта | +5 сек после старта | `startup-pulse` DateTrigger (одноразово) |
| 12 | Самопроверка | Каждые 5 мин | `health-selfcheck` cron `minute="*/5"` |
| 13 | Чистка audit_log (152-ФЗ) | Ежедневно 04:15 | `audit-log-retention` cron `hour=4, minute=15` |
| 14 | Удаление неактивных операторов | Ежедневно 04:20 | `stale-operators-cleanup` cron `hour=4, minute=20` |
| 15 | Обновление кэша threat-intel URL | Ежечасно :17 | `threat-intel-refresh` cron `minute=17` |
| 16 | Закрытие зависших DRAFT-рассылок | Ежечасно :37 | `broadcast-draft-reaper` cron `minute=37` |
| 17 | Внешний healthcheck ping | Каждые 5 мин, если задан URL | `healthcheck-ping` добавляется только при `HEALTHCHECK_URL` |

В `services/cron.build_scheduler` всегда регистрируются 16 задач. `healthcheck-ping` добавляется семнадцатой задачей только при заданном `HEALTHCHECK_URL`. Раньше в Регламенте было 13 — новые cron фиксируются как известное расхождение v7 и должны попасть в проект Регламента v8.
