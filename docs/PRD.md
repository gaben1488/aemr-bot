...
**Доступность.** Целевое время безотказной работы — 99% на минимально жизнеспособном продукте. Бот развёрнут на собственном сервере без публичного inbound-доступа (long polling). Самопроверка по расписанию каждые `HEALTHCHECK_INTERVAL_MIN` минут проверяет сигнал жизни и шлёт переходный алерт в админ-группу при потере отзывчивости. Compose-healthcheck стучится в `/livez` локально (`127.0.0.1:8080`), а `/readyz` и совместимый `/healthz` используются для диагностики готовности с проверкой БД. Внешний пинг включается заданием `HEALTHCHECK_URL`.

**SLA-мониторинг и pulse.** SLA = `SLA_RESPONSE_HOURS = 4` рабочих часа. Cron `overdue-reminder-workhours` (пн–пт 09:00–11:59 и 13:00–17:59, в `:40`) шлёт в служебную группу список просроченных обращений. Если просроченных нет — сообщение не шлётся. Pulse — два расписания: `pulse-hourly` (каждый час 24/7 в `:05`, базовый heartbeat) и `pulse-workhours-extra` (пн–пт 09:00–17:59 в `:35`, дополнительный пинг в рабочее время). Плюс однократный `startup-pulse` через 5 секунд после старта. Все расписания заданы APScheduler-кронами с таймзоной `Asia/Kamchatka`. Дополнительные cron'ы: `audit-log-retention` (ежедневно 04:15), `stale-operators-cleanup` (ежедневно 04:20), `threat-intel-refresh` (ежечасно `:17`), `broadcast-draft-reaper` (ежечасно `:37`).
