...
**Ф-19. Регистрация команд в /-меню MAX.** На старте бот вызывает `bot.set_my_commands(...)` со списком из 8 команд жителя: `/start`, `/menu`, `/help`, `/policy`, `/subscribe`, `/unsubscribe`, `/forget`, `/whoami`. Операторские команды в /-меню не регистрируются — MAX API не поддерживает scope-разделение по чатам, и засорять `/`-подсказку жителя командами `/reply`, `/erase` и прочими нельзя.
...
| 11 | Регистрация команд через `set_my_commands` | closed — регистрируется только список жителя (8 команд), операторские команды текстом и кнопками |
...
| 14 | Scope-разделение команд между жителем и оператором | open — MAX API сейчас не поддерживает per-chat scope для `set_my_commands`. Ждать улучшения API либо принять как есть: операторские команды отсутствуют в `/`-меню жителя и оператора, доступ — по тексту команды и через кнопки `/op_help` |
...
**2026-05-08. Операторские команды убраны из /-меню MAX.** В `set_my_commands` идёт только список жителя (8 команд). Операторские `/reply`, `/erase`, `/setting`, `/broadcast` и прочие в /-подсказке не появляются. Причина: MAX API не поддерживает scope-разделение команд по чату; иначе житель в личке видел бы /-меню оператора и наоборот. Открытый вопрос #14 — ждать улучшения API.
