...
Источники проверены против HEAD `d4e4e28` (2026-05-09):
...
| `texts.py:LOCALITY_REQUEST` | «Выберите населённый пункт:» | Эта константа **не используется** (после `NAME_RECEIVED.format(name=...)`). Мёртвый код в словаре сообщений. | Удалить или явно пометить deprecated |
...
| `texts.py:TOPIC_RECEIVED`, `APPEAL_CARD_TEMPLATE`, `ADMIN_CARD_TEMPLATE`, `ADDRESS_RECEIVED` | «Тема» vs «Тематика» — разнобой | В echo «✓ Тема: …», в карточках «Тематика: …», в подсказке «Выберите тематику». Один объект — три названия. | Унифицировать: «**Тема**» везде в текстах (короче, привычнее). «Тематика» оставить только в комментариях кода. |
