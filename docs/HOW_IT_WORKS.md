...
Доступен поиск жителя по телефону или MAX user id командой
`/find_resident` — каждый запрос пишется в audit_log (152-ФЗ §17).
...
диагностикой, резервными копиями (`/backup`). Подменю «📊 Аудитория и
согласия» доступно только ИТ — это master-listing с пагинацией по 10
жителей на страницу, поиском по любому атрибуту (имя, телефон, MAX id)
и bulk dump'ом отдельной карточкой на каждого.
...
- **pulse-hourly** — каждый час 24/7 в `:05`. Базовый heartbeat «процесс жив», уведомление не оператору, а внешнему мониторингу.
- **pulse-workhours-extra** — пн–пт 09:00–17:59 в `:35`. Дополнительный heartbeat в рабочее время, ловит зависание между основными пинами.
- **audit-log-retention** — ежедневно в 04:15, чистит `audit_log` старше 365 дней (152-ФЗ).
- **stale-operators-cleanup** — ежедневно в 04:20, помечает покинувших служебную группу операторов как неактивных.
- **threat-intel-refresh** — ежечасно в `:17`, обновляет URLhaus/ThreatFox/PhishTank host-set для предупреждений оператору.
- **broadcast-draft-reaper** — ежечасно в `:37`, переводит DRAFT-рассылки старше 30 минут в статус FAILED.
