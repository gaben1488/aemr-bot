...
3. **Настроить внешний uptime-monitor** (UptimeRobot / Healthchecks.io) на публично доступный `/healthz` либо использовать `HEALTHCHECK_URL` (heartbeat-pulse). Без внешнего пингера никто не узнает о падении бота, кроме как от обиженных жителей.
...
6. **Проверить, что VPS физически в РФ** (152-FZ ст. 18.5).
