...
- **Fail-closed (SEC #7)**: при любой ошибке записи idempotency-ключа (DB stall, transient timeout) событие **отбрасывается**, не обрабатывается. До SEC #7 был fail-open («default to process»), что позволяло attacker'у через induced DB stall реплеить мутирующие callback'и.
