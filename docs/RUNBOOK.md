...
**Житель «попрощался, но дождаться ответа» (👋 в Настройки → Уйти из бота).** Это **штатный сценарий** (Вариант 3, утверждено 2026-05-09). У жителя `consent_revoked_at IS NOT NULL`, но обращения, поданные **до** отзыва, остаются открытыми. Оператор отвечает обычным образом — кнопкой «✉️ Ответить» под карточкой или `/reply N <текст>`. Это «прощальный» ответ: после доставки обращение → CLOSED, через 30 дней без активности retention-cron автоматически обезличит данные жителя. **Не закрывайте через `/close` без ответа** — житель ждёт ответ перед уходом.

**Житель «стёр данные прямо сейчас» (❌ в Настройки → Уйти из бота) или ИТ выполнил `/erase`.** Все его открытые обращения **уже закрыты** автоматически с `closed_due_to_revoke=true` — оператор не успеет ответить. На карточке таких обращений кнопки «🔁 Возобновить» нет (бесполезна — гард доставки в `_deliver_operator_reply` всё равно откажет). Если в админ-чате попытаться ответить — увидите «⚠️ Не могу доставить ответ … житель отозвал согласие на ПДн». Это нормально, ничего делать не нужно.
