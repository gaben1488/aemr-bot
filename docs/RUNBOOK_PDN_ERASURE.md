...
- `users.first_name` заменяется на `Удалено`;
- `users.phone` и `users.phone_normalized` очищаются;
- `users.consent_pdn_at` очищается;
- `users.dialog_state` возвращается в `idle`;
- `users.dialog_data` заменяется на пустой объект;
- `users.subscribed_broadcast` становится `false`;
- `users.is_blocked` становится `true`;
...
- запись `users` как технический якорь для внешних ключей;
