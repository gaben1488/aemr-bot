...
### 1.6. Визуальная структура карточки (с версии 2026-05-26)
...
4. **Что** — Населённый пункт / Адрес / Тема / Суть обращения. Если есть ссылки от жителя — здесь же визуальный warning (см. §1.5 выше).
...
### 3.5. Поиск жителя без открытия обращения (`/find_resident <max_user_id|+7XXXXXXXXXX>`, любая роль)

С версии 2026-05-27 в боте есть быстрый поиск карточки жителя — без открытия обращения, без перехода в меню «📊 Аудитория и согласия». Используйте, когда нужно перепроверить контакты для callback'а, посмотреть статус согласия на ПДн или убедиться, что житель не в blocklist'е, **до** того как открыть переписку.

**Что разрешено.** Любой оператор любой роли — координатор, AEMR, EGP, IT — может выполнить запрос. Не нужны права ИТ.

**Что бот делает автоматически.**
- Каждый запрос пишется в `audit_log` (актор, время, что искали в маскированной форме `+7***NNNN`). Это требование 152-ФЗ §17 — лог всех обращений к ПДн.
- В ответ оператору телефон отображается маскированно (`+7***NNNN`), полный номер виден только при разворачивании карточки конкретного обращения, под кнопкой «📞 Перезвонить».
- Если по телефону нашлось несколько жителей (супруги на одной симке) — бот возвращает «not-found» с подсказкой искать по `max_user_id`. Это страховка против случайного раскрытия не того жителя.

**Что НЕЛЬЗЯ.**
- Использовать `/find_resident` для запросов вне служебной задачи. Каждая запись в `audit_log` именная, IT-оператор и комплаенс-аудит периодически просматривают активность. Поиск «из любопытства» — нарушение 152-ФЗ §6 (обработка ПДн только в заявленных целях).
- Передавать результаты поиска (имя + телефон) в личку, мессенджер, по почте без необходимости. Карточка жителя — служебная информация.
- Снимать скриншоты карточки и хранить локально дольше времени выполнения задачи.

### 3.6. Если что-то пошло не так
