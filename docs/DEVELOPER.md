...
- Coverage gate на CI: `--cov-fail-under=67` (см. `.github/workflows/ci.yml:139`). Цель — поднимать постепенно до 80% (Phase E в плане MLP). Текущий реальный coverage на CI — 66.8% после Cluster E tests; gate на edge — регрессия валит CI.
