...
Электронная приёмная переехала из главного меню в подменю «Приём граждан» — там она стоит рядом с расписанием очного приёма, и житель видит обе формы обращения в одном экране. /-меню MAX очищено — `set_my_commands()` без аргументов; жители работают только через кнопочное меню.
...
| 👋 **Хочу попрощаться, но дождаться ответа** | `consent_pdn_at=NULL`, `consent_revoked_at=now`, `subscribed_broadcast=false`, `dialog_state=IDLE` | Остаются открытыми. Оператор отвечает «прощальным» ответом (`_deliver_operator_reply` пропускает по факту наличия `consent_revoked_at`, но не блокирует доставку для уже поданных) → CLOSED. Через 30 дней без активности retention-cron делает erase | Через `/start` + новое согласие. Имя/телефон сохраняются между revoke и автоудалением (если retention не сработал) |
...
Каждый шаг воронки начинается **echo-feedback** строкой «✓ \<Что\>: \<выбор\>» — пенсионер видит, что предыдущий шаг закрыт. Без echo выбор ощущался незаконченным («нажал — а оно куда-то ушло»). См. `texts.NAME_RECEIVED`, `LOCALITY_RECEIVED`, `ADDRESS_RECEIVED`, `TOPIC_RECEIVED` — все они стали format-строками `"✓ <Что>: {placeholder}\n\n<подсказка следующего шага>"`. `_ask_*` функции в `handlers/appeal.py` читают свежесохранённое значение из `user.full_name` или `user.dialog_data` и подставляют в echo.
...
flowchart TD
    ...
    Welcome[Бот: приветствие<br/>6 кнопок главного меню]:::bot
    ...
    AskSummary --> InputSummary[Житель: текст / фото / гео<br/>одним сообщением]:::citizen
    InputSummary --> CreateAppeal[(appeals.status = new<br/>locality, address, topic, summary,<br/>attachments JSONB)]:::db
...
flowchart TD
    ...
    OnSum --> Finalize[(persist_and_dispatch_appeal:<br/>создать обращение)]:::handler
    Finalize --> IdleAfterCreate[Возврат в idle]:::handler
...
stateDiagram-v2
    ...
    answered --> new: «🔁 Подать похожее»<br/>создаёт новое связанное обращение
    answered --> in_progress: /reopen N

    closed --> in_progress: /reopen N
    closed --> new: «🔁 Подать похожее»<br/>создаёт новое связанное обращение
    closed --> [*]
```
...
stateDiagram-v2
    ...
    awaiting_summary --> idle: первое непустое сообщение<br/>+ создание appeals.NNN
