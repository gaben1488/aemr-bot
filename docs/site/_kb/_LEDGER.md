# Выверенный реестр фактов — _LEDGER.md

Источник: распределённая верификация кода + ресёрч (wf_96077512-752, 2026-06-01).
КОД = истина №1. Факты с file:line. Решения владельца и Постановление №2129 учтены.

---

## ВЕРИФИКАЦИЯ — Житель/воронка

I now have everything. One final confirmation point: the progress counter M is statically `len(_STAGES)` = 5 (it does not vary with pre-filled phone/name — contact is a pre-step excluded from the counter, and `_show_progress_step` only starts rendering progress from the "name" stage onward). The geo button confirmation (RequestGeoLocationButton on the locality step) is confirmed in `localities_keyboard`. All facts verified.
...
- `DEFAULTS['appointment_text']` — `services/settings_store.py:350-355`, дословно: `"Приём граждан временно исполняющим полномочия Главы Елизовского муниципального района А.С. Гончаровым осуществляется два раза в месяц (1 и 3 среда каждого месяца) по предварительной записи. Запись на приём ведётся по номеру телефона 8 (415-31) 7-25-29."` Содержит «РАЙОНА» и «А.С. Гончаровым» — соответствует решению владельца (осознанный seed).
- `DEFAULTS['topics']` — `services/settings_store.py:357`: `"topics": []` (пустой список; реальные темы приходят из seed-файла).
- `seed/topics.json` — список тем по порядку (9 шт.): `Дороги`, `Мусор`, `Свалки`, `Благоустройство`, `Управляющие компании`, `Транспорт`, `Образование`, `Культура и спорт`, `Другое`. Заливается в БД через `seed_if_empty` (`services/settings_store.py:810-813`).
- Дополнительно: `DEFAULTS['localities']` задан прямо в коде (`:378-389`, 10 поселений: Елизовское ГП, Вулканное ГП, Корякское СП, Начикинское СП, Николаевское СП, Новоавачинское СП, Новолесновское СП, Паратунское СП, Пионерское СП, Раздольненское СП).
...
## ВЕРИФИКАЦИЯ — Оператор

I now have full verification. One nuance: `ensure_role` checks exact membership in the allowed set (line 64: `op.role not in {r.value for r in allowed}`), so IT-gated commands (`/erase`, `/setting`, `/backup`, `/add_operators`) accept ONLY the IT role — coordinator does NOT pass. The docstring in admin_resident_search.py (line 9) saying "OP/SH/IT" uses old role naming, but the actual guard `ensure_operator` accepts any active operator.

Let me do one final confirmation: that `/whoami` in start.py is citizen-side (not operator) and verify the count of `op_help_keyboard` buttons by re-examining the exact structure. The keyboard code (lines 426-446) is already fully read. Let me confirm the README's claimed counts aren't contradicted — actually I'll just compute directly from code. All facts are verified.
...
- `services/settings_store.py:527`, `handlers/admin_settings.py:1146` — «Электроэнергия / Отопление / Холодная вода» это дефолтные подписи справочных контактов для жителя (emergency_contacts), не маршрутизация важности.
- `copy/broadcast_texts.py:263`, `services/broadcast_templates.py:201` — слово «вода» как пример поискового запроса по шаблонам рассылок.
...
## ВЕРИФИКАЦИЯ — Администратор/инфраструктура

Confirmed: SCHEMA has exactly **17 keys**. All facts verified by code. Here is the report.

---

# Верификация по коду — раздел «Администратор/инфраструктура»

Все факты подтверждены по `C:/Users/filat/Documents/aemr-bot`. Источник истины — код, ссылки даны `file:line`.

## 1. Cron-задачи: полный перечень с точным расписанием

**Источник истины расписания — `services/cron.py` (объект `jobs`, строки 838–999), НЕ `cron_registry.py`.** Реестр `cron_registry.py` — машинно-читаемый anti-drift список имён для docs-sync теста, его `schedule_human` человекочитаемый и в ОДНОМ месте расходится с фактическим триггером (см. ⚠️ ниже). Расписание ниже взято из реальных `CronTrigger(...)` в `cron.py`.

| id (name) | Триггер в коде (cron.py) | TZ | Назначение | file:line |
|---|---|---|---|---|
| `db-backup` | `day_of_week=BACKUP_DAY_OF_WEEK("sun"), hour=BACKUP_HOUR(3), minute=BACKUP_MINUTE(0)` → **вс 03:00** | Asia/Kamchatka | pg_dump→GPG→том, ротация; алёрт по `fail_kind` | cron.py:840-849; config.py:93-95 |
| `events-retention` | `hour=4, minute=0` → ежедневно **04:00** | TZ | удаление `events` старше 30 дней (idempotency cleanup, PII в payload) | cron.py:851-855 |
| `audit-log-retention` | `hour=4, minute=15` → ежедневно **04:15** | TZ | удаление `audit_log` старше `AUDIT_LOG_RETENTION_DAYS` (default 365) | cron.py:857-861 |
| `broadcast-draft-reaper` | `minute=37` → **ежечасно :37** | TZ | DRAFT-рассылки старше 30 мин → FAILED (SEC F5) | cron.py:867-871 |
| `threat-intel-refresh` | `minute=17` → **ежечасно :17** | TZ | refresh URLhaus+ThreatFox(+PhishTank); critical-алёрт если stale >6ч | cron.py:877-881 |
| `stale-operators-cleanup` | `hour=4, minute=20` → ежедневно **04:20** | TZ | деактивация операторов, покинувших служебную группу MAX | cron.py:887-891 |
| `health-selfcheck` | `minute=*/HEALTHCHECK_INTERVAL_MIN` (default 5) → **каждые 5 мин** | TZ | алёрт при heartbeat fresh↔stale | cron.py:893-899 |
| `monthly-stats` | `day=1, hour=9, minute=0` → **1-го числа 09:00** | TZ | XLSX-отчёт за месяц в админ-чат | cron.py:901-905 |
| `startup-pulse` | `DateTrigger(now+5s)` → **однократно +5 сек после старта** | TZ | catch-up pulse при рестарте | cron.py:910-916 |
| `pulse-hourly` | `minute=5` → **каждый час :05, 24/7** | TZ | базовый heartbeat «бот работает» | cron.py:938-945 |
| `pulse-workhours-extra` | `day_of_week=mon-fri, hour=9-17, minute=35` → **пн–пт 09:35–17:35** | TZ | доп-пинг в рабочее время | cron.py:946-955 |
| `appeals-5y-retention` | `hour=4, minute=45` → ежедневно **04:45** | TZ | обнуление текста обращений старше 5 лет | cron.py:957-961 |
| `pdn-retention` | `hour=4, minute=30` → ежедневно **04:30** | TZ | обезличивание жителей через 30 дней после revoke (152-ФЗ ст.21 ч.5) | cron.py:962-967 |
| `funnel-watchdog` | `minute=15` → **ежечасно :15** | TZ | сброс зависших анкет воронки (>24ч); алёрт при ≥5/час | cron.py:970-974 |
| `open-reminder-workhours` | `day_of_week=mon-fri, hour="9-11,13-17", minute=10` → **пн–пт :10 (кроме обеда 12ч)** | TZ | напоминание об открытых обращениях | cron.py:985-991 |
| `overdue-reminder-workhours` | `day_of_week=mon-fri, hour="9-11,13-17", minute=40` → **пн–пт :40 (кроме обеда 12ч)** | TZ | напоминание о просроченных по SLA | cron.py:992-998 |
| `healthcheck-ping` | `minute=*/HEALTHCHECK_INTERVAL_MIN` (5) — **условно**, только если задан `HEALTHCHECK_URL` | TZ | внешний ping (Healthchecks.io/Uptime Kuma) | cron.py:1001-1009 |

Общие параметры всех job (цикл регистрации, cron.py:1011-1019): `max_instances=1`, `coalesce=True`, `misfire_grace_time=120` сек (`_MISFIRE_GRACE_SEC`, cron.py:49).
...
Эндпоинты регистрируются в `start()` (health.py:149-152):
- **`/livez`** (health.py:124-127) — liveness. Только heartbeat (`heartbeat.is_fresh()`), БД НЕ трогает (`include_db=False`). Используется Docker healthcheck, watchdog, auto-deploy gate.
- **`/readyz`** (health.py:129-131) — readiness. heartbeat + `SELECT 1` к БД (`include_db=True`, `_ping_db_cached`, TTL 10 сек). Для диагностики, не для авто-рестарта.
- **`/healthz`** (health.py:134-141) — backward-compat alias на `_readyz` (heartbeat + DB).
- Ответ 200 если `ok`, иначе **503** (health.py:121). Локальным запросам (127.0.0.1/::1/localhost, health.py:91-93) отдаётся полная диагностика (`heartbeat_fresh`, `last_beat_age_seconds`, `db_ok`); внешним — только `{"ok": ...}`.

**Порт:** дефолт сигнатуры `start(host="0.0.0.0", port=8080)` (health.py:144-147), но фактически вызывается из `main.py:385-386` с `host=settings.webhook_host, port=settings.webhook_port`. `webhook_port` = `Field(8080, alias="WEBHOOK_PORT")` (config.py:21), `webhook_host` = `"0.0.0.0"` (config.py:20). В compose наружу пробрасывается только loopback: `"127.0.0.1:8080:8080"` (docker-compose.yml:70), и Docker healthcheck бьёт `/livez` (docker-compose.yml:80).

## 4. db/models.py — таблица настроек + SCHEMA

- **`__tablename__ = "settings"`** для `class Setting(Base)` (models.py:231-232). Ожидание подтверждено — таблица называется `settings`, НЕ `app_settings`. PK — `key: String(64)` (models.py:234).
- **`settings_store.SCHEMA` — ровно 17 ключей** (settings_store.py:395-451), подтверждено импортом (`len(SCHEMA)==17`). Полный перечень: `welcome_text`, `consent_text`, `commit_author_name`, `commit_author_email`, `policy_url`, `electronic_reception_url`, `udth_schedule_url`, `udth_schedule_intermunicipal_url`, `appointment_text`, `emergency_contacts`, `transport_dispatcher_contacts`, `topics`, `broadcast_max_images`, `admin_quiet_hours_enabled`, `admin_quiet_hours_start`, `admin_quiet_hours_end`, `localities`. `DEFAULTS` тоже 17 ключей; `SYNCED_KEYS` — 9 (settings_store.py:708-718, без `commit_author_*`, `welcome_text`, `consent_text`, `broadcast_max_images`, `admin_quiet_hours_*`).

`appointment_text` дефолт (settings_store.py:350-355) содержит «Елизовского **муниципального района**» и «**А.С. Гончаровым**» — соответствует решению владельца (осознанный seed), не ошибка.
...
- **`BACKUP_KEEP_COUNT` = 8** (config.py:101, `Field(8, alias="BACKUP_KEEP_COUNT")`), комментарий «8 еженедельных ≈ 2 месяца». Ротация по mtime, удаляет файлы сверх `keep` (db_backup.py:95-109, вызов на :349).
...
- BACKUP_KEEP_COUNT — config.py:101 — 8.
...
## ВЕРИФИКАЦИЯ — Разработчик + Безопасность

All seven sections verified against code. Here are the facts.

---

## Раздел «Разработчик»

**1. Миграции Alembic — 17 файлов, последняя `0017_appeals_last_card_mid`**
`bot/aemr_bot/db/alembic/versions/` — полный список (соответствует ожиданию 0001..0017):
0001_initial, 0002_broadcast, 0003_phone_normalized, 0004_indexes_and_autovacuum, 0005_appeals_locality, 0006_consent_revoked_at, 0007_consent_broadcast_anonymous, 0008_backfill_consent_broadcast, 0009_partial_indexes_for_hot_paths, 0010_pg_ops_hardening, 0011_wizard_state_persistence, 0012_messages_appeal_created_index, 0013_settings_synced_at, 0014_broadcasts_attachments, 0015_broadcast_templates, 0016_broadcast_template_usage, 0017_appeals_last_card_mid.

**2. ORM-модели — 11, ровно как в списке**
`bot/aemr_bot/db/models.py`: User (models.py:61), Operator (:128), Appeal (:142), Message (:180), Event (:207), AuditLog (:220), Setting (:231), Broadcast (:253), BroadcastDelivery (:287), WizardState (:303), BroadcastTemplate (:337). Классы `Base` (:9) и StrEnum-перечисления (DialogState, AppealStatus, OperatorRole, MessageDirection, BroadcastStatus) — не таблицы.
...
- `--cov-fail-under=67` — `.github/workflows/ci.yml:139` (подтверждено 67).

**4. AUDIT_LOG_RETENTION_DAYS**
config.py:129-130 — дефолт **365**, диапазон `ge=30, le=3650` (30..3650 дней).
...
- Источники: **URLhaus + ThreatFox + опц. PhishTank** (если задан `PHISHTANK_APP_KEY`). threat_intel.py:41 `_URLHAUS_URL = https://urlhaus.abuse.ch/downloads/csv_online/`, :42 `_THREATFOX_URL = https://threatfox.abuse.ch/downloads/hostfile/`, :43-44 `_PHISHTANK_URL_TEMPLATE`; PhishTank активируется только при ключе (:235-241). **Kaspersky/OpenTIP ОТСУТСТВУЕТ** — подтверждено: ни одного упоминания kaspersky/opentip в threat_intel.py (задача #89 его планировала, но в коде нет; реализована только триада abuse.ch).
- `is_stale()` — threat_intel.py:100 (`age > _STALENESS_BUDGET_SEC`); вспомогательный `staleness_age_seconds()` — :90.
- Critical-алёрт при устаревании (PR #166): cron.py:326-341 — `if store.is_stale()` → `_send_admin_text_with_retry(..., context="threat-intel-stale", critical=True)`; `critical=True` пробивает quiet hours (комментарий cron.py:309-310). Job `threat-intel-refresh` зарегистрирован cron.py:878-880, обработчик `_job_threat_intel_refresh` — cron.py:294.
- NFKC-нормализация в `extract_urls`: **settings_store.py:165** `text = unicodedata.normalize("NFKC", text)` (полноширинные/совместимые символы → канон; кириллические гомоглифы NFKC не трогает — их ловит `_QUASI_URL_PATTERN`).
...
- **Внешний мониторинг (HEALTHCHECK_URL) — present-but-optional, активный код.** `healthcheck_url: str | None = Field(None, alias="HEALTHCHECK_URL")` — config.py:121. Job `healthcheck-ping` добавляется в расписание только если URL задан: `if settings.healthcheck_url:` — cron.py:1002; пинг `_ping_healthcheck` (cron.py:1059) при пустом URL делает ранний return (cron.py:1061). Интервал — `HEALTHCHECK_INTERVAL_MIN` (дефолт 5, config.py:55). Это рабочий dormant-by-default, а не reverted.
...
I have all the facts I need. There's no `[tool.uv]` section (so `package = true` default applies — the root *is* built as a wheel, which matters: `--no-editable` will build aemr-bot via setuptools and install it as a regular wheel, equivalent in behavior to the current `pip install -e` minus the editable symlink). uv 0.11.17 is current (the machine has 0.11.12). The guard test, DEPS.md, dependabot all already orbit `uv.lock`.

Now I'll write the recommendation. This is the deliverable that goes to the owner.
...
I now have every fact verified against the code and the legal norms cross-checked through КонсультантПлюс/ФСТЭК sources. The picture is unambiguous. Here is my decision.
...
`db/models.py:220-228` — таблица `audit_log` хранит **действия операторов**, не сами обращения граждан: `operator_max_user_id` (кто), `action` (что: block/unblock/reopen/close/erase/setting_update и пр.), `target` (над кем/чем), `details` JSONB (может содержать ПДн — например `setting_update` пишет before/after значений). Это **журнал доступа/действий с ПДн**, то есть журнал событий безопасности ИСПДн в смысле приказа ФСТЭК №21.

Критично для развязки: обращения граждан имеют **отдельный** ретеншн-механизм — `cron.py:565 _job_appeals_5y_retention` (5 лет, 04:45). `audit_log` (`cron.py:262`, 04:15) — это другой объект и другая задача. Старый черновик Политики п.7.5 ошибочно «согласовал» срок audit_log со сроком обращений («Срок согласован со сроком хранения обращений (пункт 7.2)», `Политика_v2.md:125`). Эта привязка юридически ложная — у двух сущностей разные правовые основания.
...
**Постановление Администрации №2129 §1.2**: при работе с сообщениями из открытых источников **59-ФЗ не применяется**. Значит 5-летний архивный срок, унаследованный из логики обращений граждан/номенклатуры дел (на него опирался `appeals_5y_retention`, `cron.py:567`), к журналу действий операторов **юридически не привязан**. ПДн — только по 152-ФЗ → принцип минимизации.
...
- параметр настраиваемый (`config.py:130`, диапазон 30–3650) — если конкретная проверка/инцидент потребует, срок поднимается через env без правки кода.
...
`config.py:129-131` уже = 365, диапазон 30–3650 корректен (нижняя граница 30 ниже ФСТЭК-минимума 90 дней, но это техническая граница «не выключить вовсе»; рабочий дефолт 365 ей не противоречит). Комментарии в коде (`config.py:123-128`, `cron.py:262-274`) и `SECURITY.md §8.1` (строки 312-314) уже описывают 365 как «год аудита, типовая глубина расследования инцидента» — совпадает с принятым обоснованием. Рассинхрон **только** в черновике `Политика_v2.md` (5 лет) — правится текст Политики, не код.

---

## Файлы (абсолютные пути)

- Канон значения: `C:\Users\filat\Documents\aemr-bot\bot\aemr_bot\config.py:129-131` (`audit_log_retention_days = 365`, ge=30 le=3650)
- Cron-задача: `C:\Users\filat\Documents\aemr-bot\bot\aemr_bot\services\cron.py:262-291` (`_job_audit_log_retention`, cutoff = now − 365 дней)
- Модель (что хранится): `C:\Users\filat\Documents\aemr-bot\bot\aemr_bot\db\models.py:220-228`
...
I now have complete coverage of both codebases. I have verified: the DB schema (appeals/messages/users/broadcasts/broadcast_deliveries/audit_log with exact columns and indexes), the existing /stats XLSX path, the SLA computation gap (the bot uses wall-clock `answered_at - created_at` while telegram_analytics has a proper `working_hours.py` business-hours calculator + RU holidays via `calendar_ru.py`), the topic taxonomy bridge (`settings_store["topics"]` free-text), the escalation scorer, situation graph, incident model, tone matrix, quality checklist, and operator templates. Let me write the specification.

The task explicitly forbids writing report .md files — I must return the full specification as my final message. This is a large deliverable, so I'll structure it precisely.
...
- лимит символов: Telegram ≤1024 (реком. 800) — у бота `answer_max_chars` сейчас **300** (`config.py:40`), это жёстче Стандарта, ок; но линтер должен предупреждать о канцелярите, не о длине.
