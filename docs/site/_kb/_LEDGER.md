...
`bot/aemr_bot/db/models.py`: User (models.py:61), Operator (:168), Appeal (:182), Message (:220), Event (:247), AuditLog (:260), Setting (:271), Broadcast (:293), BroadcastDelivery (:327), WizardState (:343), BroadcastTemplate (:377). Классы `Base` (:9) и StrEnum-перечисления (DialogState, AppealStatus, OperatorRole, MessageDirection, BroadcastStatus) — не таблицы. _(ревизия 2026-06-04: номера строк классов сдвинулись — Operator/Appeal/Message/Event/AuditLog/Setting/Broadcast/BroadcastDelivery/WizardState/BroadcastTemplate; значения и состав те же. Анти-дрейф числа 11 — `test_ledger_facts_sync.py`.)
...
**Постановление Администрации №2129 §1.2**: при работе с сообщениями из открытых источников **59-ФЗ не применяется**. Значит 5-летний архивный срок, унаследованный из логики обращений граждан/номенклатуры дел (на него опирался `appeals_5y_retention`, `cron.py:569`), к журналу действий операторов **юридически не привязан**. ПДн — только по 152-ФЗ → принцип минимизации.
