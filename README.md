...
В `docs/` лежит пять рабочих документов плюс юридический PDF. Этого набора хватает для разработки, эксплуатации, передачи проекта и ручной редактуры всех текстов бота.
...
Семь кнопок плюс ссылка на электронную приёмную, если она настроена.
...
3. **🔔 Подписаться на новости** или **🔕 Отписаться от новостей** — одна и та же кнопка, текст меняется по состоянию подписки. Подписавшимся приходят рассылки от АЕМО.
4. **🌐 Электронная приёмная** — прямая ссылка на сайт АЕМО. Показывается, если URL задан в настройках.
5. **📋 Приём граждан** — расписание и контакты.
6. **📚 Полезная информация** — подменю с телефонами экстренных и аварийных служб, муниципальными и межмуниципальными маршрутами, диспетчерскими автотранспорта.
7. **⚙️ Настройки и помощь** — подменю «Помощь и команды», «Политика данных», «Удалить мои данные» (с подтверждением).

Команды дублируют кнопки и доступны параллельно: `/start`, `/menu`, `/help`, `/policy`, `/subscribe`, `/unsubscribe`, `/forget`, `/whoami`.
