...
# --- §1 Разработчик: 23 миграции, последняя 0023 ---------------------


def test_migrations_count_is_23() -> None:
    """`_LEDGER.md` Разработчик §1: ровно 23 файла миграций.

    Считаем файлы `alembic/versions/*.py` (импортировать нечего —
    миграции это файлы). В леджере уже были дрейфы 17→18→19→20→21→22→23 — этот
    тест ловит следующий.
    """
    nums = _migration_numbers()
    assert len(nums) == 23, (
        f"_LEDGER.md фиксирует 23 миграции (0001..0023), на диске "
        f"{len(nums)}: {nums}." + _FIX
    )


def test_latest_migration_is_0023_and_contiguous() -> None:
    """`_LEDGER.md` Разработчик §1: последняя ревизия `0023`, без дыр.

    Нумерация обязана быть непрерывной 1..23 — иначе пропущена/удалена
    миграция, и «последняя 0023» в леджере вводит в заблуждение.
    """
    nums = _migration_numbers()
    assert nums[-1] == 23, (
        f"_LEDGER.md фиксирует последнюю миграцию 0023, максимальная на "
        f"диске {nums[-1]:04d}." + _FIX
    )
    assert nums == list(range(1, 24)), (
        f"_LEDGER.md подразумевает непрерывную нумерацию 0001..0023, на "
