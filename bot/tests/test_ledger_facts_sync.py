...
# --- §1 Разработчик: 21 миграция, последняя 0021 ---------------------


def test_migrations_count_is_21() -> None:
    """`_LEDGER.md` Разработчик §1: ровно 21 файл миграций.

    Считаем файлы `alembic/versions/*.py` (импортировать нечего —
    миграции это файлы). В леджере уже были дрейфы 17→18→19→20→21 — этот
    тест ловит следующий.
    """
    nums = _migration_numbers()
    assert len(nums) == 21, (
        f"_LEDGER.md фиксирует 21 миграцию (0001..0021), на диске "
        f"{len(nums)}: {nums}." + _FIX
    )


def test_latest_migration_is_0021_and_contiguous() -> None:
    """`_LEDGER.md` Разработчик §1: последняя ревизия `0021`, без дыр.

    Нумерация обязана быть непрерывной 1..21 — иначе пропущена/удалена
    миграция, и «последняя 0021» в леджере вводит в заблуждение.
    """
    nums = _migration_numbers()
    assert nums[-1] == 21, (
        f"_LEDGER.md фиксирует последнюю миграцию 0021, максимальная на "
        f"диске {nums[-1]:04d}." + _FIX
    )
    assert nums == list(range(1, 22)), (
        f"_LEDGER.md подразумевает непрерывную нумерацию 0001..0021, на "
