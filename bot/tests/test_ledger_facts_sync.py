...
# --- §1 Разработчик: 20 миграций, последняя 0020 ---------------------


def test_migrations_count_is_20() -> None:
    """`_LEDGER.md` Разработчик §1: ровно 20 файлов миграций.

    Считаем файлы `alembic/versions/*.py` (импортировать нечего —
    миграции это файлы). В леджере уже были дрейфы 17→18→19→20 — этот
    тест ловит следующий.
    """
    nums = _migration_numbers()
    assert len(nums) == 20, (
        f"_LEDGER.md фиксирует 20 миграций (0001..0020), на диске "
        f"{len(nums)}: {nums}." + _FIX
    )


def test_latest_migration_is_0020_and_contiguous() -> None:
    """`_LEDGER.md` Разработчик §1: последняя ревизия `0020`, без дыр.

    Нумерация обязана быть непрерывной 1..20 — иначе пропущена/удалена
    миграция, и «последняя 0020» в леджере вводит в заблуждение.
    """
    nums = _migration_numbers()
    assert nums[-1] == 20, (
        f"_LEDGER.md фиксирует последнюю миграцию 0020, максимальная на "
        f"диске {nums[-1]:04d}." + _FIX
    )
    assert nums == list(range(1, 21)), (
        f"_LEDGER.md подразумевает непрерывную нумерацию 0001..0020, на "
