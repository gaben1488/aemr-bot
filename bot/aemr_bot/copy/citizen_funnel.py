...
# Запасной текст согласия. Показывается, только если настройка
# consent_text в БД пуста (боевой текст оператор задаёт через админ-UI —
# см. docs/_soglasie_final.md). Даже как запасной он не должен врать
# жителю: раньше здесь стояло «Отзыв — /forget», но /forget — это
# немедленное СТИРАНИЕ данных, а не отзыв согласия (реальный отзыв — в
# меню «Уйти из бота»). Держим фактически верным и полным по ч. 1 ст. 9.
CONSENT_REQUEST = (
    "Чтобы принять обращение, Управлению делами Администрации "
    "Елизовского муниципального округа нужно ваше согласие на обработку "
    "персональных данных.\n\n"
    "Мы сохраним: имя, телефон, идентификатор вашего аккаунта в MAX, "
    "населённый пункт и адрес проблемы, тему, текст и вложения "
    "обращения, переписку по нему. Только чтобы принять обращение, "
    "передать специалисту и ответить, а также для отчётности.\n\n"
    "Согласие действует, пока вы не отзовёте его: «Уйти из бота» → "
    "«Хочу попрощаться, но дождаться ответа». После отзыва данные "
    "удаляем в течение 30 дней.\n\n"
    "Не пишите в обращении о здоровье, вере, политических взглядах — "
    "мы их не запрашиваем.\n\n"
    "Полная политика — {policy_url}\n\n"
...
CONTACT_RECEIVED = "Спасибо. Как к вам обращаться?"
...
APPEAL_LIST_EMPTY = (
    "Вы ещё не отправляли обращений. Нажмите «📝 Написать обращение» "
    "ниже — это пара минут."
)
...
ADDRESS_EMPTY = (
    "Укажите адрес проблемы (не ваш домашний адрес). Например: "
    "г. Елизово, ул. Ленина, д. 13."
)
...
SUBSCRIBE_MINI_CONSENT = (
    ...
    "и важных работах.\n\n"
    "Информировать жителей о чрезвычайных ситуациях и работе коммунальных "
    "служб — обязанность органа местного самоуправления (Федеральный закон "
    "№ 68-ФЗ «О защите населения и территорий от чрезвычайных ситуаций», "
    "статья 11; Федеральный закон № 131-ФЗ об общих принципах организации "
    "местного самоуправления, статья 16).\n\n"
