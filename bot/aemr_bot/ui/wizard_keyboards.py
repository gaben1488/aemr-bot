...
# Подписи взяты из Регламента (часть 43) — оператор читает документ и
# работает в боте, словарь должен быть один. «ЕГП» сохранено как
# историческое обозначение: Администрация Елизовского городского
# поселения упразднена при преобразовании района в округ, а роль
# соответствует специалисту территориального управления (часть 4
# Регламента). Расшифровка в скобках — чтобы новый оператор не гадал,
# что значат три буквы.
ROLE_LABELS: dict[str, str] = {
    OperatorRole.IT.value: "🛠 it — специалист отдела цифрового развития",
    OperatorRole.COORDINATOR.value: "👤 coordinator — координатор АЕМО",
    OperatorRole.AEMR.value: "👤 aemr — специалист АЕМО (округ)",
    OperatorRole.EGP.value: "👤 egp — специалист ЕГП (территориальное управление)",
}
