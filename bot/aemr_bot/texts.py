WELCOME = (
    "Здравствуйте. Это чат-бот Администрации Елизовского "
    "муниципального округа.\n\n"
    "Здесь Вы можете сообщить о проблеме, задать вопрос "
    "и найти полезную информацию.\n\n"
    "🔔 Подпишитесь на новости округа, чтобы не пропустить "
    "объявления о ЧС, отключениях и работах коммунальных служб — "
    "кнопка «Подписаться на новости» в меню ниже.\n\n"
    "Выберите действие."
)

CONSENT_REQUEST = (
    "Перед оформлением обращения нужно ваше согласие на обработку персональных "
    "данных в соответствии с 152-ФЗ.\n\n"
    "Полный текст политики: {policy_url}\n\n"
    "Нажмите «Согласен», чтобы продолжить."
)

CONSENT_DECLINED = (
    "Без согласия на обработку персональных данных принять обращение не можем. "
    "Вы можете вернуться в меню и продолжить позже."
)

CONSENT_ACCEPTED = "Спасибо. Поделитесь номером — он нужен, чтобы перезвонить по обращению."

CONTACT_REQUEST = (
    "Чтобы связаться по обращению, поделитесь номером — кнопка ниже. "
    "Имя подтянем из вашего профиля в MAX. Если в профиле имени нет — спросим отдельно."
)

CONTACT_RECEIVED = "✅ Контакт получен. Как к вам обращаться?"

LOCALITY_REQUEST = "Выберите населённый пункт:"

NAME_RECEIVED = "✅ Имя записано. Выберите населённый пункт."

LOCALITY_RECEIVED = "✅ Населённый пункт записан. Укажите адрес (улица, дом)."

ADDRESS_RECEIVED = "✅ Адрес записан. Выберите тематику обращения."

TOPIC_RECEIVED = (
    "✅ Тематика выбрана. Опишите суть обращения одним сообщением. "
    "Можно прикрепить фото или файл — пришлите их вместе с текстом."
)

APPEAL_ACCEPTED = (
    "✅ Обращение #{number} принято в работу.\n"
    "Обычно ответ приходит в течение одного рабочего дня."
)

APPEAL_LIST_EMPTY = "Вы ещё ничего не отправляли. Нажмите «📝 Написать обращение» в меню — это пара минут."

APPEAL_EMPTY_REJECTED = (
    "Чтобы отправить, нужно описать проблему текстом или прикрепить фото. "
    "Например: «Не вывозят мусор по ул. Ленина, 13». "
    "Без описания обращение не примем."
)

NAME_EMPTY = "Введите ваше имя. Например: Алексей."

ADDRESS_EMPTY = "Укажите адрес проблемы. Например: г. Елизово, ул. Ленина, д. 13."

CONTACT_RETRY = (
    "Номер телефона не принимается текстом — нужно нажать кнопку. "
    "Нажмите «Поделиться контактом» ниже."
)

APPEAL_CARD_TEMPLATE = (
    "📄 Обращение #{number}\n"
    "📅 Дата: {created_at}\n"
    "📊 Статус: {status_emoji} {status_label}\n"
    "📍 Населённый пункт: {locality}\n"
    "🏠 Адрес: {address}\n"
    "🏷️ Тематика: {topic}\n"
    "📝 Суть:\n{summary}"
)

ADMIN_CARD_TEMPLATE = (
    "📨 Новое обращение #{number}\n"
    "👤 Имя: {name}\n"
    "📞 Телефон: {phone}\n"
    "──────────\n"
    "📍 Населённый пункт: {locality}\n"
    "🏠 Адрес: {address}\n"
    "🏷️ Тематика: {topic}\n"
    "📝 Суть:\n{summary}\n"
    "──────────\n"
    "💬 Как ответить:\n"
    "• Свайп-ответ на это сообщение или «Ответить» в меню — затем текст.\n"
    "• Или команда: /reply {number} <ваш текст>\n"
    "Лимит — {answer_limit} символов."
)

ADMIN_FOLLOWUP_TEMPLATE = (
    "💬 Дополнение к обращению #{number}\n"
    "👤 {name}\n"
    "──────────\n"
    "{text}"
)

ADMIN_REPLY_DELIVERED = (
    "✉️ Ответ ушёл жителю.\n"
    "Обращение #{number} закрыто."
)

CITIZEN_REPLY_TEMPLATE = (
    "📬 Ответ Администрации Елизовского муниципального округа "
    "на Ваше обращение\n\n"
    "#{number} от {created_at}\n"
    "🏷️ Тема: {topic}\n"
    "📍 Населённый пункт: {locality}\n"
    "🏠 Адрес: {address}\n"
    "──────────\n"
    "{reply_text}"
)

ADMIN_REPLY_TOO_LONG = (
    "⚠️ Ответ длиннее {limit} символов (сейчас {actual}). "
    "Жителю ничего не ушло. Сократите и пришлите ещё раз."
)

ADMIN_REPLY_NO_APPEAL = (
    "⚠️ Не понял, к какому обращению ответ. "
    "Сделайте реплай на карточку."
)

CONTACTS_MENU_TITLE = "Выберите раздел:"

USEFUL_INFO_TITLE = "📚 Полезная информация — выберите раздел:"

SETTINGS_MENU_TITLE = (
    "⚙️ Настройки и помощь.\n\n"
    "Здесь можно посмотреть список команд, открыть политику обработки "
    "персональных данных и удалить свои данные из системы."
)

ERASE_CONFIRM = (
    "🗑️ Удалить мои данные?\n\n"
    "Будут стёрты: имя, телефон, история обращений останется в архиве "
    "обезличенной. Согласие на обработку персональных данных будет "
    "отозвано, рассылка отключена. Действие необратимо."
)

ERASE_REQUESTED = "Принято. Удалим ваши данные в течение суток."

CANCELLED = "Действие отменено. Открываю главное меню."

UNKNOWN_INPUT = (
    "Не понял ваше сообщение. Откройте /menu, чтобы выбрать действие, "
    "или /help — список команд."
)

HELP_USER = (
    "Команды:\n"
    "/menu — главное меню\n"
    "/help — эта справка\n"
    "/policy — политика обработки персональных данных (PDF)\n"
    "/subscribe — подписаться на новости АЕМО\n"
    "/unsubscribe — отписаться от новостей\n"
    "/forget — запросить удаление моих данных"
)

SUBSCRIBE_BUTTON_ON = "🔔 Подписаться на новости"
SUBSCRIBE_BUTTON_OFF = "🔕 Отписаться от новостей"

SUBSCRIBE_CONFIRMED = (
    "Готово, Вы подписаны на новости Администрации Елизовского "
    "муниципального округа. Отписаться можно в любой момент — "
    "командой /unsubscribe или кнопкой под рассылкой."
)
SUBSCRIBE_ALREADY_ON = "Вы уже подписаны на новости. /unsubscribe — отписаться."
UNSUBSCRIBE_CONFIRMED = "Подписка отключена. Вернуть — командой /subscribe."
UNSUBSCRIBE_ALREADY_OFF = "Вы не подписаны. /subscribe — подписаться."

BROADCAST_HEADER = "📢 Объявление от Администрации Елизовского муниципального округа"
BROADCAST_UNSUBSCRIBE_BUTTON = "🔕 Отписаться от рассылки"

POLICY_DELIVERED = (
    "Политика обработки персональных данных Администрации Елизовского "
    "муниципального округа — во вложении."
)

POLICY_FALLBACK_URL = (
    "Политика обработки персональных данных:\n{policy_url}"
)

POLICY_UNAVAILABLE = (
    "Сейчас не получается отправить файл политики. Попробуйте позже или "
    "напишите координатору Администрации Елизовского муниципального округа."
)

STATUS_LABELS = {
    "new": ("🆕", "Новое"),
    "in_progress": ("🔄", "В работе"),
    "answered": ("✅", "Завершено"),
    "closed": ("⛔", "Закрыто без ответа"),
}

# Operator-side texts ---------------------------------------------------------

OP_HELP = (
    "🛠️ Памятка оператора\n\n"
    "Как ответить жителю:\n"
    "• Способ 1 — reply: свайп влево на карточке обращения (или меню "
    "«Ответить»), пишете текст, отправляете. Лимит 300 символов.\n"
    "• Способ 2 — команда: /reply <номер> <текст>. Например: "
    "/reply 42 Здравствуйте, асфальт включён в план ремонта.\n\n"
    "Команды для операторов:\n"
    "/reply N <текст> — ответить жителю по обращению N\n"
    "/stats today|week|month — выгрузка XLSX\n"
    "/reopen N — вернуть обращение в работу\n"
    "/close N — закрыть без ответа\n"
    "/broadcast — рассылка подписавшимся (coordinator/it)\n"
    "/broadcast list — последние рассылки\n"
    "/erase max_user_id=N — удалить ПДн жителя по id (it)\n"
    "/erase phone=+7... — удалить ПДн жителя по телефону (it)\n"
    "/setting list — список настроек (it)\n"
    "/setting <key> <value> — изменить настройку (it)\n"
    "/add_operators — bulk-регистрация операторов (it)\n"
    "/backup — снять pg_dump прямо сейчас (it)\n"
    "/diag — диагностика бота\n"
    "/open_tickets — показать список открытых обращений. Ответы - по свайпу"
)

OP_ADD_OPERATORS_USAGE = (
    "Используйте:\n"
    "/add_operators\n"
    "<max_user_id> <role> <ФИО>\n"
    "<max_user_id> <role> <ФИО>\n\n"
    "Роли: coordinator, aemr, egp, it.\n"
    "Пример:\n"
    "/add_operators\n"
    "123456 coordinator Иванова Анна Петровна\n"
    "789012 aemr Петров Пётр Иванович"
)

OP_ADD_OPERATORS_RESULT = (
    "✅ Добавлено: {added}\n"
    "🔁 Обновлено: {updated}\n"
    "⚠️ Ошибок: {errors}"
)

OP_NOT_AUTHORIZED = (
    "Эта команда — только для операторов в служебной группе. "
    "Если вы должны быть оператором, попросите координатора "
    "зарегистрировать вас через /add_operators."
)
OP_APPEAL_NOT_FOUND = "Обращение #{number} не найдено."
OP_APPEAL_REOPENED = "Обращение #{number} возвращено в работу."
OP_APPEAL_CLOSED = "Обращение #{number} закрыто без ответа."
OP_USER_ERASED = "Данные пользователя {max_user_id} удалены."
OP_SETTING_UPDATED = "Настройка «{key}» обновлена."
OP_SETTING_NOT_FOUND = (
    "Настройка «{key}» не найдена. Доступные ключи — в /setting list. "
    "Например: policy_url, sla_response_hours, electronic_reception_url."
)
OP_STATS_EMPTY = "За выбранный период обращений не было."

# Broadcast / рассылка ---------------------------------------------------------

OP_BROADCAST_PROMPT = (
    "Введите текст рассылки одним сообщением. Лимит {limit} символов. "
    "/cancel чтобы отменить."
)
OP_BROADCAST_TOO_LONG = (
    "Текст длиннее {limit} символов (сейчас {actual}). "
    "Сократите и пришлите ещё раз."
)
OP_BROADCAST_CANCELLED_BY_USER = "Подготовка рассылки отменена."
OP_BROADCAST_NO_SUBSCRIBERS = (
    "Подписчиков нет, рассылать некому. /broadcast — попробовать позже."
)
OP_BROADCAST_PREVIEW = (
    "📢 Предпросмотр:\n──────────\n{text}\n──────────\n"
    "Разослать {count} подписчикам?"
)
OP_BROADCAST_STARTED = "Рассылка #{number} запущена.\n⏳ 0/{total}"
OP_BROADCAST_PROGRESS = (
    "Рассылка #{number}\n"
    "⏳ {delivered}/{total} доставлено{failed_suffix}"
)
OP_BROADCAST_FAILED_SUFFIX = ", {failed} не доставлено"
OP_BROADCAST_DONE = (
    "✅ Рассылка #{number} завершена.\n"
    "Доставлено: {delivered} из {total}.{failed_line}"
)
OP_BROADCAST_FAILED_LINE = "\nНе доставлено: {failed}."
OP_BROADCAST_CANCELLED = (
    "⛔ Рассылка #{number} остановлена.\n"
    "Доставлено до остановки: {delivered} из {total}."
)
OP_BROADCAST_LIST_EMPTY = "Рассылок ещё не было."
OP_BROADCAST_LIST_HEADER = "Последние рассылки:\n"
OP_BROADCAST_LIST_ITEM = "#{number} · {created_at} · {status} · {delivered}/{total}"
OP_BROADCAST_WIZARD_EXPIRED = (
    "Ввод текста занял слишком долго, мастер закрыт. /broadcast — начать заново."
)
