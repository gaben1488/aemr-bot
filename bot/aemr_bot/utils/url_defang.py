...
# `_BARE_DOMAIN_PATTERN` (косметический defang 2-го уровня для показа
# оператору), здесь ловим и поддомены — `secure.vk-gosuslugi.top`,
# `www.evil.com` — чтобы фильтр исходящих ссылок нельзя было обойти лишним
# поддоменом.
#
# SECURITY (P2, 2026-07-16): детекция для гейта НАМЕРЕННО НЕ привязана к
# курируемому `_DEFANG_TLDS`. Раньше «голый» хост на непокрытом TLD
# (`.click`/`.link`/`.zip`/`.mobi`/`.pro`/`.life`/`.cyou`/`.buzz`/`.fun`
# и сотни других) не извлекался и НЕ проверялся по whitelist — фишинговая
# ссылка под официальной шапкой уходила жителю. Гейт — это whitelist
# (`_is_whitelisted_host` отсеивает не-гос), поэтому ДЕТЕКЦИЯ должна быть
# широкой, а решение принимает whitelist. Ловим любой host-подобный токен
# `label.label(.label)*`, где последний лейбл (TLD):
#   • ЛИБО ASCII-буквенный длиной ≥2 (`[a-z]{2,63}`) — покрывает ЛЮБОЙ
#     gTLD/ccTLD-хвост без перечисления, список-независимо;
#   • ЛИБО один из немногих не-ASCII / punycode TLD рунка (`рф` и его
#     punycode-формы `xn--p1ai`/`xn--p1acf`/`xn--80aswg`), которые нельзя
#     выразить как `[a-z]{2,}`.
# Требование «TLD буквенный, ≥2» отсекает числа/версии/IP/время
# (`1.2.3`, `192.168.1.1`, `8.00`) и однобуквенные сокращения (`т.д.`,
# `и.о.`, `п.2`) — они НЕ считаются хостами.
#
# Про не-ASCII набор: перечисление именно этой закрытой группы IDN-TLD
# рунка — не «привязка к курируемому списку детекции», а наоборот, то, что
# ПРЕДОТВРАЩАЕТ ложные срабатывания. Кириллический TLD по паттерну
# неотличим от кириллического слова, поэтому «широкая» кириллица ловила бы
# рус. предложения без пробела после точки (`день.Меня`). ASCII-часть —
# полностью список-независимая и несёт всю безопасность (все фишинговые
# gTLD там); не-ASCII-часть — минимальный закрытый набор ccTLD.
_GATE_SPECIAL_TLDS = tuple(t for t in _DEFANG_TLDS if not re.fullmatch(r"[a-z]{2,}", t))
_BARE_HOST_PATTERN = re.compile(
    ...
    r"(?:[a-z]{2,63}|"  # широкий ASCII-TLD (список-независимо)
    + "|".join(re.escape(t) for t in _GATE_SPECIAL_TLDS)  # + не-ASCII/punycode
    + r"))"
...
# Заведомо файловые суффиксы, которые НЕ существуют как реальные gTLD:
# «справка.pdf», «фото.jpg» в исходящем тексте — это имена файлов, а не
# домены (MAX их не авто-линкует, кликабельной ссылки не возникает).
# Исключаем их из хост-детекции гейта, чтобы штатный ответ оператора
# «направьте справку.pdf» не придерживался как «непроверенная ссылка».
# ВАЖНО: сюда НЕ входят суффиксы, которые одновременно реальные gTLD и
# используются в фишинге (`.zip`, `.mov` — gTLD от Google): они остаются
# детектируемыми. Это не «курируемый список TLD для детекции» (безопасность
# по-прежнему широкая), а список не-доменных расширений — обратная задача.
...
def extract_bare_hosts(text: str) -> list[str]:
    ...
    Токены с заведомо файловым расширением (`_FILE_EXT_NOT_TLD`)
    пропускаются — это имена файлов, не домены.
    """
    if not text:
        return []
    seen: set[str] = set()
