...
class Settings(BaseSettings):
    ...
    # Проверка пригодности бэкапа (services/backup_verify.py). Раз в месяц
    # свежайший дамп расшифровывается ПОТОКОМ (на диск ничего не пишется,
    # БД не трогается) и сверяется сигнатура pg_dump. Еженедельный backup
    # доказывает лишь, что файл записался; эта проверка — что его можно
    # ОТКРЫТЬ: ловит потерю парольной фразы (обесценивает все копии разом)
    # и обрезанный файл (у GPG есть контроль целостности). Стоит копейки —
    # чтение потока в никуда, поэтому включена по умолчанию.
    # Полное восстановление в БД сюда намеренно НЕ вынесено: оно требует
    # места под распаковку (в контейнере /tmp — tmpfs 128 МБ) и создаёт
    # временную базу на боевом диске. Это ручная квартальная процедура
    # (см. «Резервное копирование» в вике).
    backup_verify_enabled: bool = Field(True, alias="BACKUP_VERIFY_ENABLED")
    # 1-е число месяца, 04:30 — после воскресного бэкапа (03:00) и до
    # рабочего дня; в 04:30 сервер простаивает.
    backup_verify_day: int = Field(1, alias="BACKUP_VERIFY_DAY", ge=1, le=28)
    backup_verify_hour: int = Field(4, alias="BACKUP_VERIFY_HOUR", ge=0, le=23)
    backup_verify_minute: int = Field(
        30, alias="BACKUP_VERIFY_MINUTE", ge=0, le=59
    )
    # Потолок на расшифровку потока. Щедро: gpg читает весь файл ради
    # контроля целостности, а диск на self-host бывает медленным.
    backup_verify_timeout_seconds: float = Field(
        600.0, alias="BACKUP_VERIFY_TIMEOUT_SECONDS", gt=0.0, le=7200.0
    )
    ...
    # Отдельный, более длинный срок для записей, подтверждающих
    # УНИЧТОЖЕНИЕ ПДн (см. cron.ERASURE_ACTIONS). Такая запись — не след
    # действия, а доказательство исполнения обязанности: именно ею
    # подтверждается, что после отзыва согласия данные удалены в срок
    # (152-ФЗ ст. 21 ч. 5). Хранить доказательство меньше, чем требуется
    # его предъявлять, — значит остаться в момент проверки ни с чем.
    # 3 года (1095 дней) — по требованиям к подтверждению уничтожения
    # ПДн при автоматизированной обработке; решение владельца 2026-07-17.
    # Нижняя граница совпадает с audit_log_retention_days: срок для
    # подтверждений не может быть короче общего.
    audit_erasure_retention_days: int = Field(
        1095, alias="AUDIT_ERASURE_RETENTION_DAYS", ge=365, le=3650
    )
