...
# номер службы».
#
# Допускаем: цифры, пробелы, плюс, скобки, дефис, точка, а также запятую
# и точку с запятой — камчатские контакты часто перечисляют НЕСКОЛЬКО
# номеров в одном поле («8 (415-31) 2-00-22, 6-11-02»), это легитимный
# формат, и без запятой такой контакт молча не грузился (seed отклонялся).
# Минимум 2 символа — стандартные коды экстренных служб России («01»,
# «02», «03», «112»). Максимум 60 (запас под 2–3 номера через запятую).
# Буквы по-прежнему запрещены — премиум-ник/email/произвольный текст не пройдёт.
