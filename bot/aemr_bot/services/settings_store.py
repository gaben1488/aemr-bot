...
# номер службы».
#
# Это DISPLAY-валидатор contact-полей (emergency_contacts,
# transport_dispatchers), НЕ для номера жителя — тот матчится через
# users._normalize_phone (срезает всё кроме цифр, приводит 8/+7 к одному виду).
# Здесь задача мягче: пропустить ЛЮБУЮ реальную запись российского номера и
# отсечь произвольный текст/премиум-ник.
#
# Нумерация РФ: префикс 8 или +7 ВЗАИМОЗАМЕНЯЕМЫ; код зоны переменной длины +
# местный номер = 10 цифр после префикса (Камчатка: Петропавловск 4152 + 6
# цифр, Елизово 41531 + 5 цифр). Запись варьируется: со скобками и без,
# с тире/пробелами/точками и без. Спецномера 01/02/03/112 — 2–3 цифры,
# толл-фри 8-800. Одно contact-поле организации может содержать более одного
# номера (диспетчерская с двумя линиями: «8 (415-31) 2-00-22, 6-11-02») —
# отсюда запятая и «;».
#
# Класс символов: цифры, пробел, + ( ) - . , ; — и ни одной буквы
# (anti-injection M4: ник/email/«звоните нам» не пройдут). Длина 2..60:
# от «01» до поля с двумя-тремя номерами и форматированием.
_PHONE_PATTERN = re.compile(r"^[\d\s\+\-\(\)\.,;]{2,60}$")
...
DEFAULTS: dict[str, Any] = {
    ...
    "appointment_text": (
        # 2026-08-07: «района» → «округа» решением владельца (ADR-15,
        # дополнение) в связи с уставом Елизовского муниципального
        # округа. Дефолт действует только для НОВЫХ установок: на проде
        # значение живёт в БД и обновляется через меню «Настройки бота».
            ...
            for it in value:
                ...
                if not _is_valid_phone(phone):
                    return False, (
                        ...
                        f"(несколько номеров через запятую — можно), длина 2–60."
                    )
    if expected is int:
