...
async def list_consented(
    ...
    Из выборки выпадают заблокированные — это единый признак «нельзя
    связываться» (#270). Служебная запись-заглушка и легаси-строки
    стёртых жителей заблокированы (миграция 0024), поэтому сюда не
    попадают. По имени не фильтруем: `NULL != 'Удалено'` в SQL даёт
    NULL, и житель без имени молча исчезал из выборки.
    """
    res = await session.scalars(
        select(User)
...
async def find_pending_pdn_retention(
    ...
    *,
    ...
    """Жители, у которых нужно уничтожить ПДн по сроку 152-ФЗ ст. 21 ч. 5.

    Условия отбора:
    - consent_revoked_at не NULL и старше `days_after_revoke` дней —
      согласие отозвано и 30-дневный срок истёк;
    - consent_pdn_at IS NULL — жителю НЕ дано свежее согласие после
      отзыва (иначе основание обработки снова живо, уничтожать нельзя).

    Уничтожение (erase_pdn_detailed) физически удаляет строку users,
    поэтому «уже уничтоженные» жители в таблице просто отсутствуют —
    отдельного признака «обезличен» не нужно. Раньше здесь стоял фильтр
    `first_name != 'Удалено'` (наследие старой механики, когда
    обезличивание = переименование в «Удалено»). Он был вреден дважды:
    (1) NULL-небезопасен — `NULL != 'Удалено'` в SQL даёт NULL, не TRUE,
    поэтому житель с пустым first_name (отдал телефон, но ушёл до ввода
    имени) НИКОГДА не попадал в отбор и его телефон висел бессрочно,
    без алёрта — тихо ломался единственный верно обоснованный срок в
    проекте; (2) имя перестало быть признаком стёртого — им стала
    блокировка. Само значение «Удалено» из проекта не исчезло: с ним
    создаётся служебная запись-заглушка (`get_anonymous_user_id`), на
    которую переподвешиваются обращения уничтоженного жителя. Заглушка
    заблокирована, легаси-строки прежней механики блокирует миграция
    0024 — поэтому проверять имя больше нигде не нужно. Убран.

    Открытые обращения этого жителя должны быть обработаны до
    уничтожения: проверка делается на стороне вызывающего кода
    ...
    res = await session.scalars(
        ...
        .where(
            ...
            # не должен его уничтожить. set_consent обнуляет
    ...
    name_rows = (
        await session.scalars(
            ...
            .where(
                ...
                # Стёртого выражает блокировка (см. миграцию 0024), а не
                # строка имени: сравнение с 'Удалено' NULL-небезопасно.
                User.is_blocked.is_(False),
            )
            .order_by(User.updated_at.desc())
            .limit(limit)
        ...
        phone_rows = (
            await session.scalars(
                ...
                .where(
                    ...
                    # Признак «нельзя связываться» — блокировка, не имя.
                    # `first_name != 'Удалено'` был NULL-небезопасен: у
                    # жителя, отдавшего телефон и ушедшего до ввода имени,
                    # first_name = NULL, а `NULL != 'Удалено'` в SQL даёт
                    # NULL — строка выпадала, и оператор не находил
                    # человека по его же номеру.
                    User.is_blocked.is_(False),
                )
                .order_by(User.updated_at.desc())
                .limit(limit)
