...
def is_workday(d: date) -> bool:
    """True, если SLA/reminder-логика должна работать: пн–пт и не праздник.

    Суббота и воскресенье — выходные для операторских напоминаний.
    Технический heartbeat (`pulse-hourly`) идёт отдельно 24/7 и не
    зависит от производственного календаря. Праздники — даты из
    `seed/holidays.json`, включая переносы (8 марта на пн, 9 мая на пн
    и т.п.).
    """
