...
кнопку «Подписаться на рассылку». Из списка получателей автоматически
исключаются заблокированные (`is_blocked = true`) — это единый признак
«с жителем нельзя связываться». Стирание данных (`/erase`) удаляет
запись физически, легаси-строки прежней механики блокирует миграция
0024, служебная запись-заглушка создаётся заблокированной.

Проверять имя (`first_name != 'Удалено'`) здесь НЕЛЬЗЯ: в SQL
`NULL != 'Удалено'` даёт NULL, а не TRUE, поэтому житель, отдавший
телефон и ушедший до ввода имени, молча выпадал из оповещений о ЧС.
См. `count_subscribers` и `list_subscriber_targets`.
"""
